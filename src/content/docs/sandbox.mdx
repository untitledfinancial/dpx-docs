...
**Step 1 — Oracle gate.** Before any payment can execute, the 11-layer Stability Oracle checks current macro conditions. An `UNSTABLE` result halts settlement automatically — the same gate that runs in production.
...
The MCP server exposes 85 tools covering oracle status, compliance, quoting, and settlement. Any MCP-compatible host (Claude Desktop, Cursor) can call them directly.
